...
while True:
    try:
        ...
        if 500 > my_usdt:
            ...
            now_qty20 = (my_usdt/now_close)/1500
        elif 10000 < my_usdt:
            now_qty20 = (my_usdt/now_close)/1500
        else:
            pass
        ...
        buy_size_d = buy_size-abs(sell_size)
        sell_size_d = abs(sell_size)-buy_size
        ...
        #구간별 주문수량계산
        e_1=round(10*(now_qty20),0) 
        e_2=round(10*(now_qty20),0) 
        e_3=round(10*(now_qty20),0) 
        e_4=round(10*(now_qty20),0) 
        e_5=round(10*(now_qty20),0) 
        e_6=round(10*(now_qty20),0) 
        e_7=round(12*(now_qty20),0) 
        e_8=round(12*(now_qty20),0) 
        e_9=round(12*(now_qty20),0) 
        e_10=round(12*(now_qty20),0) 
        e_11=round(13*(now_qty20),0) 
        e_12=round(13*(now_qty20),0) 
        e_13=round(13*(now_qty20),0) 
        e_14=round(14*(now_qty20),0) 
        e_15=round(14*(now_qty20),0) 
        e_16=round(15*(now_qty20),0) 
        e_17=round(16*(now_qty20),0) 
        e_18=round(20*(now_qty20),0) 
        e_19=round(46*(now_qty20),0) 
        e_20=round(54*(now_qty20),0) 
        e_21=round(73*(now_qty20),0) 
        e_22=round(91*(now_qty20),0) 
        e_23=round(119*(now_qty20),0) 
        e_24=round(150*(now_qty20),0) 
        e_25=round(196*(now_qty20),0) 
        e_26=round(234*(now_qty20),0) 
        e_27=round(295*(now_qty20),0) 
        e_28=round(353*(now_qty20),0) 
        e_29=round(452*(now_qty20),0) 
        e_30=round(579*(now_qty20),0) 
        e_31=round(746*(now_qty20),0) 
        e_32=round(962*(now_qty20),0) 
        e_33=round(1244*(now_qty20),0) 
        e_34=round(1610*(now_qty20),0) 
        e_35=round(1932*(now_qty20),0) 
        e_36=round(2318*(now_qty20),0) 
        e_37=round(2782*(now_qty20),0) 
        e_38=round(3338*(now_qty20),0) 
        e_39=round(4006*(now_qty20),0) 
        ...
        #주문하기    
        #공매수 주문
        if now_close < average_5_20 and count_side != "Buy" and buy_size <= 1000 : # 현재가가 볼린전 중단(20일이평선)을 하향 돌파시
            print(client.LinearOrder.LinearOrder_cancelAll(symbol=ticker).result()) # 모든주문취소


                   
            
            qty_l=[e_1,e_2,e_3,e_4,e_5,e_6,e_7,e_8,e_9,e_10,e_11,e_12,e_13,e_14,e_15,e_16,e_17,e_18,e_19,e_20,e_21,e_22,e_23,e_24,e_25,e_26,e_27,e_28,e_29,e_30,e_31,e_32,e_33,e_34,e_35,e_36,e_37,e_38,e_39,e_40]
            price_l=[p_1,p_2,p_3,p_4,p_5,p_6,p_7,p_8,p_9,p_10,p_11,p_12,p_13,p_14,p_15,p_16,p_17,p_18,p_19,p_20,p_21,p_22,p_23,p_24,p_25,p_26,p_27,p_28,p_29,p_30,p_31,p_32,p_33,p_34,p_35,p_36,p_37,p_38,p_39,p_40]
        ...
        if now_close < average_5_20-0.0014 and count_side != "Buy" and time_s == 0 and buy_size > 1000: # 현재가가 볼린전 중단(20일이평선)을 하향 돌파시
            print(client.LinearOrder.LinearOrder_cancelAll(symbol=ticker).result()) # 모든주문취소
        ...
        #공매도 주문
        if now_close >= average_5_20 and count_side != "Sell" and sell_size <=1000 : # 현재가가 볼린전 중단(20일이평선)을 상향 돌파시
            print(client.LinearOrder.LinearOrder_cancelAll(symbol=ticker).result()) # 모든주문취소
        ...
        if now_close >= average_5_20+0.0014 and count_side != "Sell" and time_s == 0 and sell_size > 1000: # 현재가가 볼린전 중단(20일이평선)을 상향 돌파시
            print(client.LinearOrder.LinearOrder_cancelAll(symbol=ticker).result()) # 모든주문취소
        ...
        #지정가 청산주문
        if buy_size_d > 0:
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=buy_size,price = round(buy_entry_price+buy_j3,4),time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
        else:
            pass
        if sell_size_d > 0:
            print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=sell_size,price = round(sell_entry_price-buy_j3,4),time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass
        ...
        # print(now_close)
        # print(a)
        print(round(sell_entry_price-buy_j3,4))
        print("실행중")
