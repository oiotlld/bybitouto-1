...
while True:
    try:
        ...
        # 주문하기
        if time_m ==0 and (time_s ==0 or time_s ==1 or time_s ==2 ) and my_date_len != 0:
            print(session.cancel_all_active_orders(symbol=ticker))
        else:
            pass
        # 공매수
        if my_date_len == 0  and size == 0 and bb_l_60 >= now_close_1 and size<my_qty*5 and en_p >= now_close_1:
        ...
        # 공매도
        if my_date_len == 0 and size == 0  and round((bb_u_60*(0.997)),4) <= now_close_1  and size<my_qty*6 and en_p <= now_close_1:
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60-(0.0001)),4),qty=qty_40,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0029)),4),qty=qty_40*1.2,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0058)),4),qty=qty_40*1.44,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0088)),4),qty=qty_40*1.73,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0117)),4),qty=qty_40*2.07,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0147)),4),qty=qty_40*2.49,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0177)),4),qty=qty_40*2.99,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0226)),4),qty=qty_40*3.58,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0275)),4),qty=qty_40*4.30,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0324)),4),qty=qty_40*5.16,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0373)),4),qty=qty_40*6.19,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0422)),4),qty=qty_40*7.43,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0471)),4),qty=qty_40*8.92,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0518)),4),qty=qty_40*10.70,time_in_force="GoodTillCancel"))
            print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0564)),4),qty=qty_40*12.84,time_in_force="GoodTillCancel"))
            # print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0611)),4),qty=qty_40*15.41,time_in_force="GoodTillCancel"))
            # print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0467)),4),qty=qty_40*1.2,time_in_force="GoodTillCancel"))
            # print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0496)),4),qty=qty_40*1.2,time_in_force="GoodTillCancel"))
            # print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0525)),4),qty=qty_40*1.2,time_in_force="GoodTillCancel"))
            # print(session.place_active_order(symbol=ticker,side="Sell",order_type="Limit",price=round((bb_u_60*(1.0554)),4),qty=qty_40*1.2,time_in_force="GoodTillCancel"))
        ...
        # # #각 포지션 평균단가 가저오기
        # # # 내 포지션 정보가저오기
        my_infor_btc=session.my_position(symbol="BTCUSD")
        # print(my_infor_btc)        
        ...
        # print(entry_price)
        # 내자산가저오기
        myxrp_btc=session.get_wallet_balance(coin="BTC")
        my_equity_btc= float(myxrp_btc["result"]["BTC"]['equity'])

        # 현재가가저오기
        now_close_btc=session.orderbook(symbol="BTCUSD")
        now_close_1_btc=float(now_close_btc["result"][1]["price"])
        # 자산/현재가
        my_qty_btc=(my_equity_btc*now_close_1_btc)/1.3
        my_qty_1_btc=round(my_equity_btc*now_close_1_btc,0)

        # 주문내역 받아보기
        my_data_1_btc=session.get_active_order(symbol="BTCUSD",order_status="New")
        ...
        if my_qty_1_btc > size_btc and round(my_qty_btc-size_btc,0) > 5 and size_btc != 0:
            print(session.place_active_order(symbol="BTCUSD",side="Sell",order_type="Market",price=round((now_close_1_btc+(0.0002)),4),qty=round(my_qty_1_btc-size_btc,0),time_in_force="GoodTillCancel"))
