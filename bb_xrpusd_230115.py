...
while True:
    try:
        ...
        # 자산/현재가
        my_qty=(my_equity*now_close_1)/1.3
        ...
        # 주문하기
        if time_m ==0 and (time_s ==0 or time_s ==1 or time_s ==2 or time_s ==3 or time_s ==4):
            print(session.cancel_all_active_orders(symbol=ticker))
        else:
            pass
        # 공매수
        if my_date_len == 0  and bb_l_60 >= now_close_1 and size<my_qty*5 and en_p >= now_close_1:
        ...
        # 공매도
        if my_date_len == 0  and bb_u_60 <= now_close_1  and size<my_qty*6 and en_p <= now_close_1:
