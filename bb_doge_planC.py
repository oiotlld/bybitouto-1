...
while True:
    try:
       ...
       #========================================각 포지션별 주문 갯수 구하기=========================================================================
        ...
        elif buy_size_d >= gty_c and buy_size > sell_size and my_leverage != 50 and my_stop==0 and buy_entry_price < now_close and sell_cls==0 and math.ceil((buy_size_d)/1000) != math.ceil(buy_close2/1000):
        ...
        elif sell_size_d >= gty_c  and buy_size < sell_size and my_leverage != 50 and my_stop==0 and sell_entry_price > now_close and buy_cls==0 and math.ceil((sell_size_d)/1000) != math.ceil(sell_close2/1000)  : 
    ...
    # 수량이 차면 양매수를 걸어라
        if buy_size_d > gty_c*3 and my_leverage != 50 and my_stop==0:
            ...
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Market",qty=round((buy_size_d)/60,0) ,price =round(now_close+0.0001,4),time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
           
         
        elif sell_size_d > gty_c*3 and my_leverage != 50 and my_stop==0:
            # 1배이상 싸이면 반대포지션을 지정가로 양매수 걸어라 초당1배수량제외한 나머지 차이의 20분에1을 걸어라
            print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Market",qty=round((sell_size_d)/60,0) ,price =round(now_close+0.0001,4),time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
