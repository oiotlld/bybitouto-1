...
while True:
    try:
        ...
        if time_s == 0 or time_s==1 : # 초단위 카운팅 0 = 1분,  공매수 공매도 둘중하나라도 위험수량에 도달하면 매수주문 안들어감
            ...
            if sell_size < 0.05 or buy_size < 0.05:
        ...
        # #     pass
         ...
         #양매수 시장가 청산하기    
        if close_all_3 > 0.003 and (buy_size < 0.1 or sell_size < 0.1):
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol="BTCUSDT",order_type="Market",qty=close_b_1,price = buy_entry_price,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass

        if close_all_3 > 0.003 and (buy_size < 0.1 or sell_size < 0.1):
            print(client.LinearOrder.LinearOrder_new(side="Buy",symbol="BTCUSDT",order_type="Market",qty=close_s_1,price = sell_entry_price,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass


         #양매수 시장가 청산하기    
        if close_haf_3 > 0.001 and (buy_size > 0.1 or sell_size > 0.1):
