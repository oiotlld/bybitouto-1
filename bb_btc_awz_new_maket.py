...
while True:
    try:
        ...
        if buy_size < sell_size and buy_size != 0 and sell_size != 0:
            close_all_1 = (now_close - buy_entry_price)*buy_size +  (now_close - sell_entry_price)*buy_size 
            close_all_2 = (buy_entry_price*buy_size) + (sell_entry_price*buy_size)
            close_all_3 = close_all_1 / close_all_2 #수익률 계산식
            close_b = buy_size
            close_s = buy_size
        
        elif buy_size >= sell_size and buy_size != 0 and sell_size != 0:
            close_all_1 = (now_close - buy_entry_price)*sell_size +  (now_close - sell_entry_price)*sell_size
        ...
        # if buy_size < sell_size and buy_size != 0 and sell_size != 0:
        #     close_haf_1 = (now_close - buy_entry_price)*buy_size +  (now_close - sell_entry_price)*(buy_size/2)
        ...
        # elif buy_size >= sell_size and buy_size != 0 and sell_size != 0:
        #     close_haf_1 = (now_close - buy_entry_price)*(sell_size/2) +  (now_close - sell_entry_price)*sell_size
        ...
        if now_close >= buy_entry_price+buy_j3+10:
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol="BTCUSDT",order_type="Market",qty=buy_size,price = buy_entry_price,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass
        
        if now_close <= sell_entry_price-sell_j3-10:
            print(client.LinearOrder.LinearOrder_new(side="Buy",symbol="BTCUSDT",order_type="Market",qty=sell_size,price = sell_entry_price,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass
