...
while True:
    try:
        ...
        # if buy_size < sell_size and buy_size != 0 and sell_size != 0:
        #     close_haf_1 = (now_close - buy_entry_price)*buy_size +  (sell_entry_price - now_close)*(buy_size/2)
        #     close_haf_2 = (buy_entry_price*buy_size) + (sell_entry_price*(buy_size/2))
        #     close_haf_3 = close_haf_1 / close_haf_2 #수익률 계산식
        #     close_b = buy_size
        #     close_s = buy_size/2
        
        # elif buy_size >= sell_size and buy_size != 0 and sell_size != 0:
        #     close_haf_1 = (now_close - buy_entry_price)*(sell_size/2) +  (sell_entry_price - now_close)*sell_size
        #     close_haf_2 = (buy_entry_price*(sell_size/2)) + (sell_entry_price*sell_size)
        #     close_haf_3 = close_haf_1 / close_haf_2 #수익률 계산식
        #     close_b = sell_size/2
        #     close_s = sell_size
        ...
        if time_s == 0 or time_s==1 : # 초단위 카운팅 0 = 1분,  공매수 공매도 둘중하나라도 위험수량에 도달하면 매수주문 안들어감
            ...
            if sell_size < 0.05 :
                print(client.LinearOrder.LinearOrder_new(side="Sell",symbol="BTCUSDT",order_type="Limit",qty=round(buy_size/3,3)-round(sell_size/3,3),price = buy_entry_price+buy_j1,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
                print(client.LinearOrder.LinearOrder_new(side="Sell",symbol="BTCUSDT",order_type="Limit",qty=round(buy_size/3,3)-round(sell_size/3,3),price = buy_entry_price+buy_j2,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
                print(client.LinearOrder.LinearOrder_new(side="Sell",symbol="BTCUSDT",order_type="Limit",qty=round(buy_size-(round(buy_size/3,3)+round(buy_size/3,3)),3)-round(sell_size/3,3),price = buy_entry_price+buy_j3,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
            else:
                pass
            if buy_size < 0.05 :
                print(client.LinearOrder.LinearOrder_new(side="Buy",symbol="BTCUSDT",order_type="Limit",qty=round(sell_size/3,3)-round(buy_size/3,3),price = sell_entry_price-sell_j1,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
                print(client.LinearOrder.LinearOrder_new(side="Buy",symbol="BTCUSDT",order_type="Limit",qty=round(sell_size/3,3)-round(buy_size/3,3),price = sell_entry_price-sell_j2,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
                print(client.LinearOrder.LinearOrder_new(side="Buy",symbol="BTCUSDT",order_type="Limit",qty=round(sell_size-(round(sell_size/3,3)+round(sell_size/3,3)),3)-round(buy_size/3,3),price = sell_entry_price-sell_j3,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
