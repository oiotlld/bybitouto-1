...
while True:
    try:
        ...
        if 500 > my_usdt:
            ...
            now_qty20 = round((my_usdt*now_close)/300,0)
        elif 10000 < my_usdt:
            now_qty20 = round((10000*now_close)/300,0)
        else:
            pass
        ...
        if (h_close <= average20 and now_open >= average20)  and (time_s == 0 or time_s==1) and buy_size < gty_d: # 현재가가 볼린전 중단(20일이평선)을 하향 돌파시
        ...
        if h_close >= average20 and now_open <= average20  and (time_s == 0 or time_s==1) and  sell_size < gty_d: # 현재가가 볼린전 중단(20일이평선)을 상향 돌파시
            print(client.LinearOrder.LinearOrder_cancelAll(symbol=ticker).result()) # 모든주문취소
            ...
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=5832*(now_qty20),price =a_1+(0.0075) ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
