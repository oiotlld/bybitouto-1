...
while True:
    try:
        ...
        if 500 > my_usdt:
            ...
            now_qty20 = round((my_usdt/now_close)/2500,0)
        elif 10000 < my_usdt:
            now_qty20 = round((10000/now_close)/2500,0)
        else:
            pass
        ...
        if time_s == 0 or time_s==10 or time_s==20 or time_s==30 or time_s==40 or time_s==50:
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=buy_size_4,price = bb_u,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
            
            print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=sell_size_4,price = bb_l,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass
        ...
        if now_close >= buy_entry_price+0.0002 and buy_size_d > gty_c:
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Market",qty=buy_size/2 ,price = buy_entry_price,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass

        if now_close <= sell_entry_price-0.0002 and sell_size_d > gty_c:
            print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Market",qty=sell_size/2,price = sell_entry_price,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
        else:
            pass

        if now_close >= bb_u+0.0001 and buy_size_d < gty_c:
        ...
        if now_close <= bb_l-0.0001 and sell_size_d < gty_c:
