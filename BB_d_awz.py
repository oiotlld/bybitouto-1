...
while True:
    try:
        ...
        if 300 > my_usdt:
            now_qty20 = 1 
        elif 300 < my_usdt and bb_cc < 0.05:
            now_qty20 = round((my_usdt*now_close)/50,0)
        elif 300 < my_usdt and bb_cc > 0.05:
            now_qty20 = round((my_usdt*now_close)/70,0)
        else:
            pass

         # 위험수량 결정 자본금의 3.5배 이상      
        gty_d = round((my_usdt/now_close)*3.5,0)
        ...
        #매도 3분할매도
        sell_size_3 = round(sell_size/3.1,0)
        buy_size_3 = round(buy_size/3.1,0)


        if time_s == 0  : # 초단위 카운팅 0 = 1분
            ...
            if (round(abs(0.0020/now_close),4) < buy_geb or round(abs(0.0022/now_close),4) <sell_geb) and (sell_size > gty_d or buy_size > gty_d):
            ...
            elif (round(abs(0.0016/now_close),4) < buy_geb or round(abs(0.0018/now_close),4) <sell_geb) and (sell_size > gty_d or buy_size > gty_d):
                price_2=[0.0001,0.0002,0.0003,0.0004,0.0005,0.0006,0.0007,0.0008,0.0009,0.001,0.0011,0.0012,0.0013,0.0014,0.0015,0.0016]                
            elif (round(abs(0.0012/now_close),4) < buy_geb or round(abs(0.0014/now_close),4) <sell_geb) and (sell_size > gty_d or buy_size > gty_d):
                price_2=[0.0001,0.0002,0.0003,0.0004,0.0005,0.0006,0.0007,0.0008,0.0009,0.001,0.0011,0.0012] 
            elif (round(abs(0.0009/now_close),4) < buy_geb or round(abs(0.0009/now_close),4) <sell_geb) and (sell_size > gty_d or buy_size > gty_d):
                price_2=[0.0001,0.0002,0.0003,0.0004,0.0005,0.0006,0.0007,0.0008,0.0009]
            elif (round(abs(0.0005/now_close),4) < buy_geb or round(abs(0.0005/now_close),4) <sell_geb) and (sell_size > gty_d or buy_size > gty_d):
            ...
            if buy_size >= gty_d:
                ...
                for n in price_2:
                    print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=buy_size/20,price = buy_entry_price - n ,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
            ...
            if sell_size >= gty_d:
                for m in price_2:
                    print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=sell_size/20,price = sell_entry_price + m ,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
            
           
            else:
                pass

            if buy_size != 0 and buy_size < gty_d :
                pp=[0,0.0001,0.0002]
                for m in pp:
                    print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=buy_size_3,price = buy_j+m ,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
                
            else:
                pass

            if sell_size != 0 and sell_size < gty_d :
                pp=[0,0.0001,0.0002]
                for m in pp:
                    print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=sell_size_3,price = sell_j-m ,time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
        ...
        if time_s == 12 or time_s == 22  or time_s == 32  or time_s == 42  or time_s == 52  or time_s == 2  : # 초단위 카운팅 0 = 1분
            #청산 주문 취소하기
            if query_1 == 1:
                my_id1 = query_3[0]
