        ...
        if time_s == 0 or time_s==1: # 초단위 카운팅 0 = 1분
            ...
            if bb_l < now_close < bb_u:
                ...
                for z, x in zip(qty_l, price_l):
                    ...
                    if now_close <= average20 and sell_size_d > gty_c and sell_entry_price < bb_l-0.001:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+100),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and buy_size_d > gty_c and buy_entry_price > bb_u+0.001:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+100),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
