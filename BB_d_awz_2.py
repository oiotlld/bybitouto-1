...
while True:
    try:
        ...
        if 300 > my_usdt:
            ...
            now_qty20 = round((my_usdt*now_close)/30,0)
        elif 300 < my_usdt and (buy_geb > 0.03 or sell_geb > 0.03) :
            now_qty20 = round((my_usdt*now_close)/20,0)
        else:
            pass
        ...
        if time_s == 0 or time_s==1: # 초단위 카운팅 0 = 1분
            ...
            if bb_l < now_close < bb_u:
                ...
                for z, x in zip(qty_l, price_l):
                    ...
                    elif now_close >= average20 and buy_size_d < gty_c:
                    ...
                    if now_close <= average20 and gty_c*2 > sell_size_d > gty_c:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+10),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and gty_c*2 >buy_size_d > gty_c:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+10),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    else:
                        pass

                    if now_close <= average20 and gty_c*4 > sell_size_d > gty_c*2:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+20),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and gty_c*4 >buy_size_d > gty_c*2:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+20),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    else:
                        pass

                    if now_close <= average20 and gty_c*8 > sell_size_d > gty_c*4:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+30),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and gty_c*8 >buy_size_d > gty_c*4:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+30),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    else:
                        pass

                    if now_close <= average20 and gty_c*16 > sell_size_d > gty_c*8:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+40),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and gty_c*16 >buy_size_d > gty_c*8:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+40),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    else:
                        pass

                    if now_close <= average20 and gty_c*32 > sell_size_d > gty_c*16:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+50),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and gty_c*32 >buy_size_d > gty_c*16:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+50),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    else:
                        pass

                    if now_close <= average20 and  sell_size_d > gty_c*32:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+60),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and buy_size_d > gty_c*32:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+60),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    else:
                        pass

            elif bb_l >= now_close or now_close >= bb_u or bb_cc > 0.05 : # 현재가가 볼린저상하단을 이탈한채 주문들어갈때 와 볼린저상하단 격차가 5%이상벌어젓을때
                qty_l=[1,2,4,8,16,32,64,128,129,130,131,132,256,257,512,513,1024,1025]
                ...
                for z, x in zip(qty_l, price_l):
                    ...
                    if now_close <= average20 and gty_c*2 > sell_size_d > gty_c:
                        print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=now_qty20*(z+10),price =a-x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    elif now_close >= average20 and gty_c*2 >sell_buy_size_dsize > gty_c:
                        print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=now_qty20*(z+10),price =a_1+x ,time_in_force="GoodTillCancel",reduce_only=False, close_on_trigger=False).result())
                    else:
                        pass
