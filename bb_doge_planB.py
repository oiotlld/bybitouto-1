    ...
    try:
       ...
       #========================================각 포지션별 주문 갯수 구하기=========================================================================
        ...
        # 동시청산주문이후 남은 포지션 평단가로 정리해서 위험회피
            ...
            print(client.LinearOrder.LinearOrder_new(side="Sell",symbol=ticker,order_type="Limit",qty=buy_size_d-buy_close2,price = round(now_close+0.0001,4),time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())
        ...
        if sell_size_d < gty_c  and math.ceil((sell_size_d)/ssall) != math.ceil(sell_close2/ssall)  and buy_size < sell_size  and my_leverage != 50 and my_stop==1 and my_tep1==1 and sell_entry_price >= now_close:
            print(client.LinearOrder.LinearOrder_new(side="Buy",symbol=ticker,order_type="Limit",qty=sell_size_d-sell_close2,price = round(now_close-0.0001,4),time_in_force="GoodTillCancel",reduce_only=True, close_on_trigger=False).result())    
